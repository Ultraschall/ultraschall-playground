...
using Ultraschall.Feed.Reader;

namespace Ultraschall.Feed
{
    public class PodcastReaderService
    {
        private readonly ILogger _logger;
        private readonly IDistributedCache _cache;

        public PodcastReaderService(ILogger logger, IDistributedCache cache)
        {
        ...
        public async Task<Podcast> GetAsync(Uri requestUri)
        {
            ...
            try
            {
                ...
                throw new PodcastReaderServiceException($"{requestUri}, {ex}");
            }
            ...
            foreach (var feed in feedlist)
            {
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    try
                    {
                        ...
                        } catch (PodcastReaderServiceException ex)
                        {
                            statMutex.WaitOne();
                            broken++;
                            statMutex.ReleaseMutex();
